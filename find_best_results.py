...
def run_model_4_3_combo_raw_unbalanced(datestamp, name):
    ...
    root_results = 'C:/Dane/Wyniki/Unbalanced_Find_Best/'
    ...
    my_callbacks = [tf.keras.callbacks.ModelCheckpoint(filepath=(results_path + 'best_weights.hdf5'), monitor="val_loss", verbose=1,
                                                       save_best_only=True)]
    ...
    best_model = load_model(filepath=(results_path + 'best_weights.hdf5'))
    results = best_model.evaluate(test_data_generator, verbose=2)
    pred = best_model.predict(test_data_generator)
    pred = np.round(pred).tolist()  # FOR BINARY
    y_test = test_data_generator.labels  # set y_test to the expected output
    results = dict(zip(best_model.metrics_names, results))
    loss = results['loss']

    with open(results_path + 'classification_report_BEST.txt', 'w') as f:
        with redirect_stdout(f):
            print(classification_report(y_test, pred, digits=4))
            print('\n\n')
            print("Loss: \t ", loss)
    ...
    ax1.set_ylabel('Stan faktyczny', fontsize=14, labelpad=8.0)
    ax1.tick_params(axis='both', labelsize=12)
    label_a = list(test_data_generator.class_indices.keys())[list(test_data_generator.class_indices.values()).index(0)]
    label_b = list(test_data_generator.class_indices.keys())[list(test_data_generator.class_indices.values()).index(1)]
    ax1.xaxis.set_ticklabels([label_a, label_b])
    ax1.yaxis.set_ticklabels([label_a, label_b])
    ax1.set_title('Macierz pomyłek', fontsize=17, pad=20.0)
    plt.savefig(results_path + 'conf_matrix_BEST.png', format='png')
    plt.savefig(results_path + 'conf_matrix_BEST.pdf', format='pdf')
...
def run_model_4_3_combo_weights(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D, BatchNormalization, Dropout
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def transfer_VGG19_weights(datestamp, name):
...
if __name__ == "__main__":
    ...
    '''
    list_of_models_names = ["MODEL_4_3_combo_raw_unbalanced", "MODEL_4_3_combo_weights", "MODEL_VGG19_weights"]

    list_of_functions = [run_model_4_3_combo_raw_unbalanced, run_model_4_3_combo_weights, transfer_VGG19_weights]
    '''
