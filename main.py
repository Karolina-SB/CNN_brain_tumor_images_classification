...
def run_model_1_2_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score

    root_results = 'C:/Dane/Wyniki/KFold_Balanced/'
    root_data = r'C:/Dane/balanced_kfold_train/'
    test_data = r'C:/Dane/balanced_kfold_test'
    train_data = pd.read_csv('C:/Users/karol/Documents/STUDIA/Untitled Folder/mgrmgr/balanced_train_labels.csv')
    ...
    def get_model(size):
        ...
        model.add(Dense(output_units, activation=activation_fun))  # Sigmoid is used for binary. Use Softmax for Mutliclass.
    ...
    #train_data_again_generator = idg.flow_from_directory(directory=root_data, target_size=set_size, class_mode=class_mode, color_mode=mode, batch_size=batch_size, shuffle=True)
    ...
    for train_index, val_index in skf.split(np.zeros(count), Y):
        ...
        history = model.fit(train_data_generator, epochs=epochs, callbacks=my_callbacks,
                            validation_data=valid_data_generator, verbose=2)
        end = time.time()
        elapsed = end - start
    ...
    print("Test Accuracy:  {:.2%} (+/- {:.2%})".format(np.mean(TEST_ACCURACY), np.std(TEST_ACCURACY)))
    print("Test Loss:      {:.4}  (+/- {:.4})".format(np.mean(TEST_LOSS), np.std(TEST_LOSS)))
    print("Test Precision: {:.2%} (+/- {:.2%})".format(np.mean(TEST_PRECISION), np.std(TEST_PRECISION)))
    print("Test Recall:    {:.2%} (+/- {:.2%})".format(np.mean(TEST_RECALL), np.std(TEST_RECALL)))
    print("Test F1:        {:.2%} (+/- {:.2%})".format(np.mean(TEST_F1), np.std(TEST_F1)))
    print("Test Time:      {:.3}  (+/- {:.3})".format(np.mean(AVR_TIME), np.std(AVR_TIME)))
    ...
    tf.keras.backend.clear_session()
    '''
    model_again = get_model(set_size)
    start = time.time()
    history = model_again.fit(train_data_again_generator, epochs=epochs, verbose=2)
    end = time.time()
    elapsed = end - start
    results = model_again.evaluate(test_data_generator, verbose=2)
    pred = model_again.predict(test_data_generator)
    pred = np.round(pred).tolist()  # FOR BINARY
    y_test = test_data_generator.labels  # set y_test to the expected output
    with open(results_path + 'GENERAL_classification_report.txt', 'w') as f:
        with redirect_stdout(f):
            print(classification_report(y_test, pred, digits=4))
            print("\n\n")
            print("Trening trwał: {:.3f} sekund".format(elapsed))
...
def run_model_2_2_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_4_1_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_4_2_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_4_4_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_5_1_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_5_2_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_5_3_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_5_4_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_6_1_kfold(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
    ...
    def get_model(size):
        ...
        model = Sequential()
        model.add(Conv2D(filters=8, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid", input_shape=(set_size[0], set_size[1], 3)))
        ...
        model.add(Conv2D(filters=256, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid"))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(filters=512, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid"))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Flatten())
        model.add(Dense(64, activation='relu'))
        model.add(Dense(32, activation='relu'))
...
def run_model_3_2_kfold_batchnorm(datestamp, name):
    from tensorflow.keras.models import Sequential
...
def run_model_3_2_kfold_dropout(datestamp, name):
    from tensorflow.keras.models import Sequential
    ...
    def get_model(size, rate1, rate2):
        tf.keras.backend.clear_session()
        lr = 0.0001  # 0.001 (default)
        opt = Adam(learning_rate=lr)
    ...
    params_dict = {'rate1': [0.0, 0.2, 0.4],
                   'rate2': [0.0, 0.2, 0.4]}



    for r1 in params_dict['rate1']:
        for r2 in params_dict['rate2']:
            print("R1: {}, R2 {}".format(r1, r2))
            folder_name = "Drop({})_Drop({})".format(r1, r2)
            ...
            for train_index, val_index in skf.split(np.zeros(count), Y):
                ...
                model = get_model(set_size, r1, r2)
...
def run_model_3_2_kfold_dropout2D(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D, SpatialDropout2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
    ...
    def get_model(size, rate1, rate2):
        ...
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid"))
        model.add(SpatialDropout2D(rate1))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid"))
        model.add(SpatialDropout2D(rate2))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Flatten())
        model.add(Dense(64, activation='relu'))
        model.add(Dense(32, activation='relu'))
...
def run_model_3_2_kfold_L2(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    ...
    def get_model(size, lamb):
        ...
        model.add(Conv2D(filters=32, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid", input_shape=(size[0], size[1], 3)))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid", kernel_regularizer=l2(lamb)))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid", kernel_regularizer=l2(lamb)))
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Flatten())
        model.add(Dense(64, activation='relu', kernel_regularizer=l2(lamb)))
        model.add(Dense(32, activation='relu'))
        model.add(Dense(output_units, activation=activation_fun))
    ...
    params_dict = {'rate': [0.0, 0.001, 0.0005, 0.0001, 0.00005, 0.00001, 0.000005, 0.000001]}



    for r1 in params_dict['rate']:

        print("R1: {}".format(r1))
        folder_name = "L2({})".format(r1)
        param_path = results_path + folder_name
        os.makedirs(param_path)
        param_path = param_path + '/'
        fold_var = 0
...
def run_model_3_2_kfold_all_regulations(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D, BatchNormalization, Dropout, SpatialDropout2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
    from tensorflow.keras.regularizers import l2
    ...
    def get_model(size):
        ...
        model.add(BatchNormalization())
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid", kernel_regularizer=l2(0.00001)))
        model.add(BatchNormalization())
        model.add(MaxPooling2D(pool_size=(2, 2)))
        model.add(Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), activation="relu", padding="valid", kernel_regularizer=l2(0.00001)))
        model.add(BatchNormalization())
...
def run_model_4_3_kfold_batchnorm(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D, BatchNormalization
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_4_3_kfold_dropout2D(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D, SpatialDropout2D
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
...
def run_model_4_3_kfold_L2(datestamp, name):
    from tensorflow.keras.models import Sequential
    from tensorflow.keras.layers import MaxPooling2D, Dense, Flatten, Conv2D
    from tensorflow.keras.regularizers import l2
    from sklearn.metrics import classification_report, recall_score, precision_score, f1_score
    ...
    params_dict = {'rate': [0.001, 0.0005, 0.0001, 0.00005, 0.00001, 0.000005, 0.000001]}
...
def transfer_InceptionV3(datestamp, name):
    ...
    def get_model(size):
        ...
        model_base = InceptionV3(include_top=False, weights='imagenet', pooling='max', input_shape=(size[0], size[1], 3))
        #model_base.trainable = False
        for layer in model_base.layers:
            layer.trainable = False
...
def transfer_ResNet50V2(datestamp, name):
    ...
    def get_model(size):
        ...
        model_base = ResNet50V2(include_top=False, weights='imagenet', pooling='max')
        #model_base.trainable = False
        for layer in model_base.layers:
            layer.trainable = False
...
def transfer_MobileNetV2(datestamp, name):
    ...
    def get_model(size):
        ...
        model_base = MobileNetV2(include_top=False, weights='imagenet', pooling='max')
        #model_base.trainable = False
        for layer in model_base.layers:
            layer.trainable = False
...
if __name__ == "__main__":
    ...
    list_of_models_names = ["transfer_VGG19_tak_tylko_na_probe"]

    list_of_functions = [transfer_VGG19]
